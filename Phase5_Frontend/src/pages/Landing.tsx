...
import {
  Shield, FileText, Code2, Zap, Lock, BarChart3, ChevronRight
} from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 text-center">
        ...
        <h1 className="text-5xl font-black text-slate-900 leading-tight mb-6">
          ...
          <span className="text-indigo-600"> Text & Code</span>
        </h1>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          ...
          <Link
            to="/login"
            className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-700 font-semibold px-6 py-3 rounded-xl border border-slate-200 transition-colors"
          >
            Sign in
          </Link>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <FeatureCard
            ...
            description="Compare essays, articles, and documents with high accuracy and get a clear similarity score instantly."
          />
          <FeatureCard
            icon={<Code2 className="w-5 h-5 text-indigo-600" />}
            title="Source Code Detection"
            description="Detect code plagiarism across Python, Java, and C++ by analyzing code structure and logic patterns."
          />
          <FeatureCard
            icon={<Zap className="w-5 h-5 text-indigo-600" />}
            title="Instant Results"
            description="Get detailed similarity reports in seconds powered by asynchronous background processing."
          />
          <FeatureCard
            icon={<BarChart3 className="w-5 h-5 text-indigo-600" />}
            title="Detailed Breakdown"
            description="Get a clear overall similarity score with visual indicators and risk level classification for every scan."
          />
          <FeatureCard
            icon={<Lock className="w-5 h-5 text-indigo-600" />}
            title="Secure & Private"
            description="Your files are stored securely and only accessible to you. Full authentication with encrypted tokens."
          />
          <FeatureCard
            icon={<Shield className="w-5 h-5 text-indigo-600" />}
            title="Submission History"
            description="Keep track of all your past scans with a full history dashboard and easy access to reports."
          />
        </div>
      </section>
      ...
      <footer className="border-t border-slate-200 px-6 py-8 mt-8">
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700">Plagiarism Detector</span>
          </div>
